...
def partition_and_get_pivot_index(array, from_index, to_index)
	...
	while pointer_a_index < pointer_b_index
		...
		while array[pointer_a_index] <= pivot 
			pointer_a_index = pointer_a_index + 1
		end


		while array[pointer_b_index] >= pivot && pointer_b_index > 0
			pointer_b_index = pointer_b_index - 1
		end
		if(pointer_a_index < pointer_b_index)
			swap(array, pointer_a_index, pointer_b_index)
		end
	end
